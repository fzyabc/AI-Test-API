...
function validateFillBusinessCodesInput(body) {
  const payload = body === undefined ? {} : ensurePlainObject(body, "fill business codes request");
  let selectedTargets = [];
  if (payload.selectedTargets !== undefined) {
    selectedTargets = ensureArray(payload.selectedTargets, "selectedTargets").map((item) => {
      const target = ensurePlainObject(item, "selectedTarget");
      const interfaceId = ensureOptionalString(target.interfaceId, "selectedTarget.interfaceId");
      const caseId = ensureOptionalString(target.caseId, "selectedTarget.caseId");
      const resultId = ensureOptionalString(target.resultId, "selectedTarget.resultId");
      if (!resultId && (!interfaceId || !caseId)) {
        throw validationError("selectedTarget must include resultId or interfaceId + caseId");
      }
      return {
        interfaceId,
        caseId,
