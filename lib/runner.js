...
const FAILURE_TYPE = {
  ...
  PRECONDITION: "precondition_fail",
};
...
function normalizeTimeoutMs(input) {
  ...
  return Math.min(Math.max(Math.floor(value), 1000), MAX_REQUEST_TIMEOUT_MS);
}

function resolveRequestTimeoutMs(settings, apiInterface, testCase, executionOptions = {}) {
  return normalizeTimeoutMs(
    executionOptions.requestTimeoutMs ||
      testCase?.requestTimeoutMs ||
      apiInterface?.requestTimeoutMs ||
...
function getAuthProfile(settings, authProfileId) {
  ...
  return (
    settings.authProfiles.find((item) => item.id === authProfileId) || null
  );
}
...
function shouldExpectSuccess(expected = {}) {
  return isExpectedSuccessBusinessCode(expected.businessCode);
}
...
function maybeBuildRetryBody(rawBody, expected, responseJson) {
  ...
  if (
    ...
    return null;
  if (
    typeof parsedBody.referralCode !== "string" ||
    !parsedBody.referralCode.trim()
  )
    return null;
...
function isLikelyEnvelopeObject(value) {
  ...
  const businessKeys = keys.filter(
    (key) => !isMetaFieldKey(key) && key !== "data",
  );
...
function extractHintKeysFromMessage(message) {
  ...
  const multiRequired = text.match(
    /([a-zA-Z_][a-zA-Z0-9_,\s]+)\s+are required/i,
  );
  if (multiRequired?.[1]) {
    multiRequired[1]
      .split(",")
...
function pickValueForTargetKey(targetKey, sourceObject) {
  ...
  const keyCandidates = entries
    .filter(
      ([key, value]) =>
        value != null &&
        typeof value !== "object" &&
        !isMetaFieldKey(normalizeFieldKey(key)),
    )
    .map(([key, value]) => ({ key, value, norm: normalizeFieldKey(key) }));

  if (targetNorm.endsWith("id")) {
    const prefix = targetNorm.slice(0, -2);
    const prefixed = keyCandidates.find(
      (item) =>
        item.norm.endsWith("id") && prefix && item.norm.includes(prefix),
    );
    if (prefixed) return prefixed.value;
    const genericId = keyCandidates.find(
      (item) => item.norm === "id" || item.norm.endsWith("id"),
    );
  ...
  if (targetNorm.includes("code")) {
    ...
    const prefixed = keyCandidates.find(
      (item) =>
        item.norm.includes("code") && prefix && item.norm.includes(prefix),
    );
    if (prefixed) return prefixed.value;
    const genericCode = keyCandidates.find(
      (item) => item.norm === "code" || item.norm.endsWith("code"),
    );
    if (genericCode && typeof genericCode.value === "string")
...
function resolveAdaptiveTargetKeys(parsedBody, messageHints) {
  ...
  for (const hint of messageHints || []) {
    ...
    const matched = bodyKeys.find((bodyKey) => {
      ...
      return (
        bodyNorm === hintNorm ||
        bodyNorm.includes(hintNorm) ||
        hintNorm.includes(bodyNorm)
      );
    });
...
function expectedMessageText(expected = {}) {
  return parseMessageIncludes(expected?.messageIncludes)
    .join(" ")
    .toLowerCase();
}
...
function shouldAllowNegativeWriteAdaptive(expected = {}) {
  ...
  return /(cannot|forbid|not allow|invalid|registration|register|inactive|disabled|frozen|pending|审核|注册|禁用|不可|不能)/.test(
    text,
  );
}
...
function scoreNegativeCandidate(expected = {}, sourceObject = {}) {
  ...
  let score = 0;
  const entries = Object.entries(sourceObject || {}).map(([key, value]) => [
  ...
  if (/(registration|register|注册)/.test(text)) {
    ...
    if (typeValue != null) {
      ...
      if (normalized === "1" || /register|registration|signup/.test(normalized))
        score += 12;
  ...
  if (/(inactive|disabled|frozen|关闭|禁用)/.test(text)) {
    const statusValue = findValue(
      /status|state|active|enabled|checkstatus|verify/,
    );
    if (statusValue != null) {
      const normalized = String(statusValue).toLowerCase();
      if (
        ["0", "1", "true", "active", "enabled", "normal", "approved"].includes(
          normalized,
        )
      )
        score -= 4;
  ...
  if (/(already default|已是默认)/.test(text)) {
    ...
    if (String(isDefault) === "1" || String(isDefault).toLowerCase() === "true")
      score += 8;
  }
...
function buildCandidateSourceInterfaces(interfacesPayload, currentInterface) {
  ...
  const interfaces = Array.isArray(interfacesPayload?.interfaces)
    ? interfacesPayload.interfaces
    : [];
  ...
  const scored = interfaces
    ...
    .map((item) => {
      ...
      const sharedSegments = pathParts.filter((segment) =>
        currentPathParts.includes(segment),
      ).length;
      let score = sharedSegments;
      if (/(list|query|get|detail|info|search)/i.test(text)) score += 3;
      if (
        path === currentPath &&
        method === String(currentInterface?.method || "").toUpperCase()
      )
        score -= 5;
      return { item, score };
    })
...
async function maybeBuildAdaptiveRetryBodies({
  ...
  if (!expectingSuccess) {
    const hasExpectedMessage =
      parseMessageIncludes(expected?.messageIncludes).length > 0;
  ...
  const sourceInterfaces = buildCandidateSourceInterfaces(
    interfacesPayload,
    apiInterface,
  );
  ...
  for (const sourceInterface of sourceInterfaces) {
    const sourcePath = replacePathParams(
    ...
    const sourceHeaders = mergeHeaders(
      sourceInterface.headers || {},
  ...
  if (!expectingSuccess) {
    return candidates
      .sort(
        (a, b) =>
          Number(b?.preflight?.negativeScore || 0) -
          Number(a?.preflight?.negativeScore || 0),
      )
...
async function executeHttpRequest({
  ...
  const timeoutId = setTimeout(() => {
    controller.abort(new Error(`Request timeout after ${normalizedTimeoutMs}ms`));
  }, normalizedTimeoutMs);
  ...
  try {
    ...
    transportError =
      aborted && !String(error?.message || "").trim()
        ? `Request timeout after ${normalizedTimeoutMs}ms`
...
function makeAssertionSummary(result) {
  ...
  const failureList = Array.isArray(result.failures) ? result.failures : [];
  return failureList.join("；");
}
...
function collectFailures({
  ...
  if (transportError) {
    ...
    return {
      failures,
      failureType: FAILURE_TYPE.TRANSPORT,
    };
  }

  if (
    shouldExpectSuccess(expected) &&
    Number(extractBusinessCode(responseJson)) >= 400 &&
    isLikelyDataPreconditionError(responseJson)
  ) {
    failures.push(
      `precondition data issue: ${responseJson?.message || "unknown data precondition error"}`,
    );
    return {
      failures,
      failureType: FAILURE_TYPE.PRECONDITION,
    };
  }

  const actualBusinessCode = extractBusinessCode(responseJson);
  if (
    expected.businessCode != null &&
    !businessCodeEquals(actualBusinessCode, expected.businessCode)
  ) {
    failures.push(
      `业务码不匹配，期望 ${expected.businessCode}，实际 ${actualBusinessCode}`,
    );
  }
  ...
  if (
    ...
    const expectedPatterns = parseMessageIncludes(
      expected.messageIncludes,
    ).join(" | ");
    failures.push(
      `response message does not include expected patterns: ${expectedPatterns}`,
    );
  }

  if (
    apiInterface.id === "referral-list" &&
    testCase.id === "list-user374-default-first" &&
  ...
  if (
    apiInterface.id === "referral-list" &&
    testCase.id === "list-user375-created-desc" &&
    responseJson?.data?.length
  ) {
    ...
    if (hasDefault) {
      ...
      for (let index = 0; index < items.length - 1; index += 1) {
        if (
          Number(items[index].createdAt) < Number(items[index + 1].createdAt)
        ) {
  ...
  if (
    apiInterface.id === "referral-code-info" &&
    testCase.id === "code-info-uppercase"
  ) {
  ...
  return {
    failures,
    failureType: failures.length ? FAILURE_TYPE.ASSERTION : null,
  };
}

function classifyFailureType(result = {}) {
  if (result.pass) return null;
  if (result.failureType) return result.failureType;
  if (result.response?.transportError) return FAILURE_TYPE.TRANSPORT;
  const failuresText = (result.failures || []).join(" ").toLowerCase();
  if (failuresText.includes("precondition data issue")) {
    return FAILURE_TYPE.PRECONDITION;
  }
  return FAILURE_TYPE.ASSERTION;
}

function buildFailureBreakdown(results = []) {
  const breakdown = {
    [FAILURE_TYPE.TRANSPORT]: 0,
    [FAILURE_TYPE.ASSERTION]: 0,
    [FAILURE_TYPE.PRECONDITION]: 0,
  };

  for (const result of results) {
    if (result?.pass) continue;
    const type = classifyFailureType(result);
    if (type && Object.prototype.hasOwnProperty.call(breakdown, type)) {
      breakdown[type] += 1;
    } else {
      breakdown[FAILURE_TYPE.ASSERTION] += 1;
    }
  }
...
async function runCase(
  ...
  if (adaptiveRetryPayloads.length) {
    for (const adaptiveRetryPayload of adaptiveRetryPayloads) {
      ...
      const interimFailureResult = collectFailures({
        transportError: requestResult.transportError,
        expected,
      ...
      if (interimFailureResult.failures.length === 0) {
        break;
      }

      if (!shouldExpectSuccess(expected)) {
        const stillPreconditionError = isLikelyDataPreconditionError(
          requestResult.responseJson,
        );
        const expectedCode = normalizeExpectedBusinessCode(
          expected?.businessCode,
        );
        const actualCode = extractBusinessCode(requestResult.responseJson);
        if (
          !stillPreconditionError ||
          (expectedCode !== null && !businessCodeEquals(actualCode, expectedCode))
        ) {
          break;
        }
      }
  ...
  const { response, responseText, responseJson, transportError } =
    requestResult;
  const failureResult = collectFailures({
    transportError,
    expected,
    response,
    responseJson,
  ...
  const failures = failureResult.failures;
