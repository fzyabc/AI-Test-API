...
function collectFailures({
  ...
  if (
    ...
    responseJson?.code !== Number(expected.businessCode)
  ) {
    failures.push(
      `业务码不匹配，期望 ${expected.businessCode}，实际 ${responseJson?.code}`,
    );
  }
  ...
  if (adaptiveRetryPayloads.length) {
    for (const adaptiveRetryPayload of adaptiveRetryPayloads) {
      ...
      if (!shouldExpectSuccess(expected)) {
        ...
        const expectedCode =
          expected?.businessCode == null ? null : Number(expected.businessCode);
        const actualCode = Number(requestResult.responseJson?.code);
        if (
          !stillPreconditionError ||
          (Number.isFinite(expectedCode) && actualCode !== expectedCode)
        ) {
          break;
        }
      }
