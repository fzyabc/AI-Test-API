...
function normalizeCase(input, apiInterface = null) {
  ...
  const expectedMeta =
    input.expectedMeta && typeof input.expectedMeta === "object"
      ? input.expectedMeta
      : {};
  return {
    id: input.id || crypto.randomUUID(),
    ...
    expectedMeta: {
      businessCodeSource: String(expectedMeta.businessCodeSource || "manual"),
      businessCodeVerified: Boolean(expectedMeta.businessCodeVerified),
      businessCodeUpdatedAt: String(expectedMeta.businessCodeUpdatedAt || ""),
    },
  };
}
