...
function buildPlanFromImportedCases(interfacesPayload) {
  ...
  const scenarios = interfaces
    .map((apiInterface, interfaceIndex) => {
      const cases = Array.isArray(apiInterface?.cases)
        ? apiInterface.cases
        : [];
      const interfaceHeaders =
        apiInterface?.headers && typeof apiInterface.headers === "object"
          ? apiInterface.headers
          : {};
      const steps = cases
        .map((testCase, caseIndex) => {
          const caseHeaders =
            testCase?.headers && typeof testCase.headers === "object"
              ? testCase.headers
              : {};
          const pathParams =
            testCase?.pathParams && typeof testCase.pathParams === "object"
              ? testCase.pathParams
              : {};
          ...
          return {
            id:
              testCase?.id ||
              `scenario-${interfaceIndex + 1}-step-${caseIndex + 1}`,
            ...
            reason: String(
              testCase?.description || apiInterface?.description || "",
            ).trim(),
          };
        })
        .filter(Boolean);
...
function normalizeScenario(scenario, scenarioIndex) {
  ...
  return {
    ...
    goal: String(
      scenario.goal || scenario.purpose || scenario.description || "",
    ),
...
function resolveStepAuthProfile(settings, executionOptions, scenario, step) {
  ...
  const overrideProfileId = String(
    executionOptions.overrideAuthProfileId || "",
  ).trim();
...
async function executeHttpStep({
  ...
  const profile = resolveStepAuthProfile(
    settings,
    executionOptions,
    scenario,
    step,
  ...
  try {
    response = await fetch(url, {
      method: step.method,
      headers,
      body: ["GET", "HEAD"].includes(step.method)
        ? undefined
        : rawBody || undefined,
    });
...
function buildJudgePrompt({
  ...
  const compactResults = results.map((item) => ({
    ...
    response: item.response?.bodyJson ||
      item.response?.bodyText || {
...
function buildMarkdownFromJudge(judgePayload, results) {
  ...
  if (!bugs.length) {
    ...
    for (const [index, bug] of bugs.entries()) {
      lines.push(
        `${index + 1}. [${bug.severity}] ${bug.title || "未命名缺陷"}`,
      );
  ...
  for (const item of results) {
    lines.push(
      `- ${item.interfaceName} / ${item.caseName}: ${item.pass ? "PASS" : "FAIL"} | ${item.assertionSummary}`,
    );
  }
  return lines.join("\n");
}
...
async function runAllWithAiAgent(
  ...
  if (
    !planPayload ||
    !Array.isArray(planPayload.scenarios) ||
    !planPayload.scenarios.length
  ) {
    ...
    if (!planResponse.ok) {
      throw new Error(
        `AI planning failed: ${planResponse.meta?.reason || "unknown"}`,
      );
    }
    const extracted = extractPlanPayload(planResponse.text);
    if (
      !extracted ||
      !Array.isArray(extracted.scenarios) ||
  ...
  if (judgePayload) {
    const judgementMap = new Map(
      judgePayload.judgements.map((item) => [item.resultId, item]),
    );
    for (const result of results) {
      ...
      result.failures =
        judged.status === "fail" ? [judged.reason || "AI 判定为业务缺陷"] : [];
      result.assertionSummary =
        judged.reason || (result.pass ? "AI 判定通过" : "AI 判定失败");
    }
  }
