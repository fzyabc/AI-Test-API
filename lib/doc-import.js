...
function normalizeExpected(expected = {}) {
  const businessCode =
    expected.businessCode == null || expected.businessCode === ''
      ? null
      : typeof expected.businessCode === 'number'
        ? (Number.isFinite(expected.businessCode) ? expected.businessCode : null)
        : String(expected.businessCode).trim() || null;
...
function sanitizeCase(input, apiInterface, authProfileIds, defaultAuthProfileId, interfaceExampleBody = null) {
  ...
  return {
    ...
    expectedMeta: {
      businessCodeSource: Object.prototype.hasOwnProperty.call(input.expected || {}, 'businessCode')
        ? 'ai_guess'
        : 'unset',
      businessCodeVerified: false,
      businessCodeUpdatedAt: '',
...
function sanitizeInterface(input, settings, options = {}) {
  ...
  const apiInterface = {
    ...
    path,
    description: String(input.description || ''),
    groupId: String(options.groupId || input.groupId || ''),
    headers: stripAuthorizationHeader(typeof input.headers === 'object' && input.headers ? input.headers : {}),
...
async function tryRepairPayloadWithAi(settings, text) {
  ...
  const repair = await callAiText(ai, {
    systemPrompt: '你是 API 导入 JSON 规范化助手，只输出 JSON。',
  ...
  if (!extracted?.interfaces?.length) {
    ...
    return {
      ...
      meta: {
        ...
        repaired: false,
        repairMeta: repaired.meta,
        reason: `AI 在 ${response.meta.endpoint} [${response.meta.wireApi}] 返回内容不是合法接口 JSON，修复失败：${repaired.reason || '未知原因'}`,
...
async function requestAiDocAnalysis(settings, doc) {
  ...
  const response = await callAiText(ai, {
    systemPrompt: '你是 API 文档分析助手，只输出严格 JSON。',
    userPrompt: prompt,
  });
  ...
  if (!analysis) {
    return {
      ...
      meta: {
        ...(response.meta || {}),
        usedFallback: true,
        reason: `AI 在 ${response.meta?.endpoint || '-'} [${response.meta?.wireApi || '-'}] 返回内容不是合法分析 JSON`,
      },
    };
  }
...
async function importApiDocument(settings, existingPayload, doc, options = {}) {
  ...
  const sanitizedInterfaces = (generation.payload.interfaces || [])
    .map((item) => sanitizeInterface(item, settings, options))
